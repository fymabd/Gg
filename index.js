...
نسعى الى خدمتكم بشكل افضل ⭐️ **`).setThumbnail(message.guild.iconURL({dynamic : true})).setTitle('SelfBot Services')],
      components: [row]
    });
  }
});
...
const randomWords = [ 'الْقَادِمَةُ' ,'حادِثَةً' ,'كَيْفً' ,'قَائِمَةٌ' ,'اللَّحْمُ' , 'ظَلامٌ' ,'إِلَى' ,'الْعَالَمُ' ,'شَاطِئٌ' ,'قَلِيلَةٌ' , 'ذَكاءٌ' ,'مُدْهِشٌ' ,'أَلْحَقِيقَةٌ' ,'مُنْخَفِضٌ' ,'مُؤْمِنٌ' , 'لِلْغَايَةِ' ,'الْمُسْتَقْبَلُ' ,'يَبْدُو' ,'سَيَّارَةٌ' ,'بَيْنَما' , 'الْحَديثَ' ,'نَصَائِحُ' ,'الْهَاتِفُ' ,'فَضَلَكَ' ,'أَتَمَنَّى' , 'مَسَاءَ' ,'مُشَكَّلَةٌ' ,'مَجْمُوعَةٌ' ,'الْفَتَاةُ' ,'صَفْحَةٌ' , 'لَقَدْ' ,'أَلِأُسْبُوعٍ' ,'عَائِلَتُي' ,'أَلِبَقاءٍ' ,'مُضَاعَفَةٌ' , 'بِالْمُنَاسَبَةِ' ,'أَثِقْ' ,'أَعْتَقِدُ' ,'إضافَةً' ,'فَضَلَكَ' , 'فِي' ,'مُوسِيقَى' ,'شُؤُونٌ' ,'هُوَ' ,'تَارِيخً' , 'مَقْرُوءٌ' ,'هُدُوءٌ' ,'سنواتٌ' ,'يَفْتَرِضُ' ,'صَغِيرٌ' , 'أَخْشَى' ,'مَفَاتِيحُ' ,'يَحْدُثُ' ,'بِخَيْرٍ' ,'بِبَعْضٍ' , 'الضَّرُورَةُ' ,'خَائِفَةٌ' ,'جَزِيرَةً' ,'الثَّالِثَةَ' ,'الْعَرَبِيَّةُ' , 'اللِّقَاءُ' ,'قَائِدٌ' ,'أَلِاِنْتِظارٍ' ,'صَحِيحٌ' ,'خَشِبَ' , 'رَغَمَ' ,'الذِّراعُ' ,'بِمُجَرَّدٍ' ,'أَلْحَقِيقَةٌ' ,'شَيْءٌ' , 'لَا' ,'قَضِيَّةٌ' ,'شَخْصِيَّةٌ' ,'أَيْضًا' ,'عَظِيمٌ' , 'الْمَنْزِلُ' ,'مَرْحَبًا' ,'تَتَّصِلُ' ,'الْغَدَاءُ' ,'شَمِس' , 'لُؤْلُؤٌ' ,'أُنْظِرُوا' ,'رَئِيسٌ' ,'مُسَاعَدَةٌ' ,'الْقَبْضُ' , 'هَادِئٌ' ,'صَدِيقِي' ,'بِخُصوصِ' ,'مِغْنَاطِيسٌ' ,'بَرْنامَجً' , 'مؤقتة' ,'مُؤَدَّبٌ' ,'دُكْتورٌ' ,'طَبِيبٌ' ,'زَواجٌ' , 'جَيِّدً' ,'عَزِيزَتُي' ,'جَمِيلَةً' ,'مَسْرُورٌ'  ] // Add your desired random words here
      ...
      await interaction.channel.send({content: `تم وضع حسابك على تلفيل بروبوت`, ephemeral: true})
      Configw.findOne({
        id: interaction.guild.id
      }, async (data) => {
        if (!data) {
...
`)]});
member.send({embeds: [new MessageEmbed().setTitle("Subscriptions 7up Services").setDescription(`تم الاشتراك في خدمة : تلفيل كتابي`)]})
...
await channel.send({content : `** الشخص الوضع التوكن ${member} \n توكن حسابك هو : ${tokennn} \n\n ايدى الروم التى سيتم فيها التلفيل هى : ${prefixxx}**`})
  ...
  clienttto.user.setActivity(`${prefixxxo}`, {type: "STREAMING", url: "https://twitch.tv/7up"}); 
      ...
      await interaction.channel.send({content: `تم اضافة الحالة الى الحساب`, ephemeral: true})
             raro.findOne({
        id: interaction.guild.id
      }, async (data) => {
        if (!data) {
...
تم الشراء من قبل : ${member}
`)]});
member.send({embeds: [new MessageEmbed().setTitle("Subscriptions 7up Services").setDescription(`تم الاشتراك في خدمة : تلفيل صوتي`)]})
              const bart = new MessageButton()
        .setLabel('قفل')
        .setCustomId('closeart')
        .setStyle('DANGER')
...
await interaction.channel.send({content: `تم رفع حسابك على فويس المحدد`, ephemeral: true})
      raro.findOne({
        id: interaction.guild.id
      }, async (data) => {
        if (!data) {
...
chan.send({embeds : [new MessageEmbed().setTitle("تلفيل صوتي").setDescription(`الحساب: ${clienttto.user.username}
تم الشراء من قبل : ${member}
`)]});
member.send({embeds: [new MessageEmbed().setTitle("Subscriptions 7up Services").setDescription(`تم الاشتراك في خدمة : تلفيل صوتي`)]})
              const bart = new MessageButton()
...
clienttto.on('messageReactionAdd', async (reaction, user) => {
    ...
    await clienttto.login(tokennnno);
      ...
      await interaction.channel.send({content: `تم وضع حسابك اوتو رياكشن`, ephemeral: true})
      ruro.findOne({
        id: interaction.guild.id
      }, async (data) => {
        if (!data) {
...
const chan = interaction.guild.channels.cache.get(cha)
const member = interaction.member
chan.send({embeds : [new MessageEmbed().setTitle("اوتو رياكشن").setDescription(`الحساب: ${clienttto.user.username}
تم الشراء من قبل : ${member}
`)]});
member.send({embeds: [new MessageEmbed().setTitle("Subscriptions 7up Services").setDescription(`تم الاشتراك في خدمة : اوتو رياكشن`)]});
      const bart = new MessageButton()
        .setLabel('قفل')
        .setCustomId('closeart')
        .setStyle('DANGER')
...
clientiii.login(tokennnno)//.then((user) => {console.log("done login " + user.user.username + " alive")})
      ...
      await interaction.channel.send({content: `تم وضع حسابك اونلاين`, ephemeral: true})
          ruro.findOne({
        id: interaction.guild.id
      }, async (data) => {
        if (!data) {
...
const chan = interaction.guild.channels.cache.get(cha)
const member = interaction.member
chan.send({embeds : [new MessageEmbed().setTitle("حالة اونلاين").setDescription(`الحساب: ${clientiii.user.username}
تم الشراء من قبل : ${member}
`)]});
member.send({embeds: [new MessageEmbed().setTitle("Subscriptions 7up Services").setDescription(`تم الاشتراك في خدمة : حساب اونلاين`)]});
      const bart = new MessageButton()
        .setLabel('قفل')
        .setCustomId('closeart')
        .setStyle('DANGER')
...
client.on('ready', async () => {
  ...
  configs.forEach(async (config) => {
    ...
    const { Client } = require('discord.js-selfbot-v13');
    const clienttt = new Client();
const randomWords = [ 'الْقَادِمَةُ' ,'حادِثَةً' ,'كَيْفً' ,'قَائِمَةٌ' ,'اللَّحْمُ' , 'ظَلامٌ' ,'إِلَى' ,'الْعَالَمُ' ,'شَاطِئٌ' ,'قَلِيلَةٌ' , 'ذَكاءٌ' ,'مُدْهِشٌ' ,'أَلْحَقِيقَةٌ' ,'مُنْخَفِضٌ' ,'مُؤْمِنٌ' , 'لِلْغَايَةِ' ,'الْمُسْتَقْبَلُ' ,'يَبْدُو' ,'سَيَّارَةٌ' ,'بَيْنَما' , 'الْحَديثَ' ,'نَصَائِحُ' ,'الْهَاتِفُ' ,'فَضَلَكَ' ,'أَتَمَنَّى' , 'مَسَاءَ' ,'مُشَكَّلَةٌ' ,'مَجْمُوعَةٌ' ,'الْفَتَاةُ' ,'صَفْحَةٌ' , 'لَقَدْ' ,'أَلِأُسْبُوعٍ' ,'عَائِلَتُي' ,'أَلِبَقاءٍ' ,'مُضَاعَفَةٌ' , 'بِالْمُنَاسَبَةِ' ,'أَثِقْ' ,'أَعْتَقِدُ' ,'إضافَةً' ,'فَضَلَكَ' , 'فِي' ,'مُوسِيقَى' ,'شُؤُونٌ' ,'هُوَ' ,'تَارِيخً' , 'مَقْرُوءٌ' ,'هُدُوءٌ' ,'سنواتٌ' ,'يَفْتَرِضُ' ,'صَغِيرٌ' , 'أَخْشَى' ,'مَفَاتِيحُ' ,'يَحْدُثُ' ,'بِخَيْرٍ' ,'بِبَعْضٍ' , 'الضَّرُورَةُ' ,'خَائِفَةٌ' ,'جَزِيرَةً' ,'الثَّالِثَةَ' ,'الْعَرَبِيَّةُ' , 'اللِّقَاءُ' ,'قَائِدٌ' ,'أَلِاِنْتِظارٍ' ,'صَحِيحٌ' ,'خَشِبَ' , 'رَغَمَ' ,'الذِّراعُ' ,'بِمُجَرَّدٍ' ,'أَلْحَقِيقَةٌ' ,'شَيْءٌ' , 'لَا' ,'قَضِيَّةٌ' ,'شَخْصِيَّةٌ' ,'أَيْضًا' ,'عَظِيمٌ' , 'الْمَنْزِلُ' ,'مَرْحَبًا' ,'تَتَّصِلُ' ,'الْغَدَاءُ' ,'شَمِس' , 'لُؤْلُؤٌ' ,'أُنْظِرُوا' ,'رَئِيسٌ' ,'مُسَاعَدَةٌ' ,'الْقَبْضُ' , 'هَادِئٌ' ,'صَدِيقِي' ,'بِخُصوصِ' ,'مِغْنَاطِيسٌ' ,'بَرْنامَجً' , 'مؤقتة' ,'مُؤَدَّبٌ' ,'دُكْتورٌ' ,'طَبِيبٌ' ,'زَواجٌ' , 'جَيِّدً' ,'عَزِيزَتُي' ,'جَمِيلَةً' ,'مَسْرُورٌ'  ];
    clienttt.on('ready', async () => {
